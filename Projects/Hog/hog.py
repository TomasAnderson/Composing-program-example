...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if outcome == 1:
            is_one_rolled = True
        num_rolls, sum = num_rolls - 1, sum + outcome
    if is_one_rolled:
        return 1
    else:
        return sum
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    def get_max_digit(num):
        max_digit = 0
        while num > 0:
            max_digit, num = max(max_digit, num % 10), num // 10
        return max_digit
    if num_rolls == 0:
        return get_max_digit(opponent_score)+1
    else:
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    """
    "*** YOUR CODE HERE ***"
    max_roll, average = 0, 0
    for num in range(1, 11):
        new_average = (average + roll_dice(num, dice) / num)
        if new_average > average:
            max_roll = num
        average = new_average
        print("%s dice scores %s on average" % (num, average))
    return max_roll
